...
class Screen(object):
    ...
    def input_stream(self):
        ...
        while True:
            ...
            if ch == curses.KEY_UP or ch == ord('k'):
                ...
                curses.endwin()
                os.system(self.items[self.current])
                pass
    ...
    def display(self):
        ...
        self.window.addstr(0, 3, "SSH CONNECTION TOOL", curses.color_pair(3))

        for idx, item in enumerate(self.items[self.top:self.top + self.max_lines]):
            # Highlight the current cursor line
            if idx == self.current:
                self.window.addstr(idx + 1, 3, item, curses.color_pair(2))
            else:
                self.window.addstr(idx + 1, 3, item, curses.color_pair(1))

        self.window.addstr(self.height - 1, 3, "BOTTOM", curses.color_pair(3))
        self.window.refresh()
...
def main():
    ...
    while True:
        line = cfg.readline()
        if not line: break
        items.append('{}'.format(line))

    # items = [f'{num + 1}. Item' for num in range(40)]
    screen = Screen(items)
    screen.run()
